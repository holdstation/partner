...
export const withdrawProvider: DataProvider = {
  // required methods
  getList: async ({
    pagination,
    filters,
    ...
    if (meta?.created_at_to) {
      metaQuery["created_at_to"] = String(
        Math.floor(dayjs(meta.created_at_to).toDate().getTime() / 1000)
      );
    }
    ...
    const filtered = Object.fromEntries(
      Object.entries(combinedQuery).filter(
